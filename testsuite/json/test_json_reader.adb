...
with VSS.JSON.Streams.Pull.Readers.Simple;

with Tests_Text_Streams;

procedure Test_JSON_Reader is

   use all type VSS.JSON.Streams.Pull.Readers.JSON_Event_Kind;
            ...
            when Invalid =>
               if Reader.Error /= Premature_End_Of_Document then
                  Ada.Text_IO.Put_Line
                    (Log_File,
                     VSS.JSON.Streams.Pull.Readers.JSON_Event_Kind'Image
                       (Reader.Event_Kind)

                     & ' '
                     & VSS.JSON.Streams.Pull.Readers.JSON_Reader_Error'Image
                         (Reader.Error)
                     & " """
